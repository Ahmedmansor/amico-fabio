const a={state:{tripId:null,apiData:null,langData:null,adults:1,children:0,addons:[],selectedAddons:new Set,formData:{name:"",nation:"",date:""}},utils:{getLang:()=>localStorage.getItem("fabio_lang")||document.documentElement.lang||"it",getI18n:t=>t==="en"?window.i18nEn||{}:window.i18nIt||{},getPrices:t=>{const n=parseFloat(t?.d_adult||t?.p_adult||0),e=parseFloat(t?.d_child||t?.p_child||0),i=parseInt(t?.min_pax||1);return{adult:n,child:e,minPax:i}},formatEUR:t=>`€${t}`,getAddonLabel:(t,n)=>n==="it"?t.label_it||t.label_en:t.label_en||t.label_it,buildExtrasListString:(t,n)=>Array.from(t.selectedAddons).map(e=>{const i=t.addons.find(l=>l.addon_id===e);return i?a.utils.getAddonLabel(i,n):e}).join(", ")},init:(t,n,e,i)=>{a.state.tripId=t,a.state.apiData=n,a.state.langData=e,a.state.addons=Array.isArray(i)?i:[],a.state.adults=parseInt(n?.min_pax||1),a.renderForm()},renderForm:()=>{const t=document.getElementById("trip-booking-form"),n=document.getElementById("btn-submit-booking");if(!t)return;const e=a.utils.getLang(),l=a.utils.getI18n(e).global||{},r=l.pricing||{},f={name:l.full_name||(e==="en"?"Full Name":"Nome Completo"),nation:l.nationality||(e==="en"?"Nationality":"Nazionalità"),date:l.trip_date||(e==="en"?"Trip Date":"Data del Viaggio"),adults:l.adults_12_plus||r.adult||(e==="en"?"Adults (12+)":"Adulti (+12)"),children:l.children_2_11||r.child||(e==="en"?"Children (2-11)":"Bambini (2-11)"),addons:l.addons_extras||(e==="en"?"Add-ons & Extras":"Extra & Supplementi"),select_country:l.select_country||(e==="en"?"Select country...":"Seleziona paese...")},d=a.utils.getPrices(a.state.apiData);let s=a._renderTextFields(f,l,e);s+=a._renderPaxControls(f,d),s+=a._renderAddons(f,e),t.innerHTML=s,a._updatePaxCounts(),a._setupDateInput(),a._setupNationPicker(f.select_country),n&&(n.onclick=()=>a.validateAndShowInvoice())},_renderTextFields:(t,n,e)=>`
    ...
    `,_renderAddons:(t,n)=>{if(a.state.addons.length===0)return"";let e=`<div class="border-t border-gray-800 my-3 pt-2"><label class="block text-xs text-gold mb-2 font-bold uppercase tracking-wider">${t.addons}</label><div class="space-y-2">`;return a.state.addons.forEach(i=>{const l=n==="it"?i.label_it:i.label_en,r=parseFloat(i.price||0);e+=`
    ...
    `,t.classList.remove("hidden")},_generateWhatsAppMessage:(t,n,e)=>{const i=n.global.whatsapp,l=t.langData?.title||t.tripId,r=document.getElementById("live-total-price"),f=r?r.textContent:"€0",d=a.utils.getLang(),s=a.utils.buildExtrasListString(t,d),o=`${t.adults} ${i.adults_word}, ${t.children} ${i.children_word}`,c={"{greeting}":i.greeting.replace("{name}",t.formData.name),"{trip}":l,"{package}":l,"{date}":t.formData.date,"{nation}":t.formData.nation||"N/A","{guests}":o,"{extras}":s||"N/A","{total}":f,"{check_availability}":e?i.package_check_availability:i.check_availability,"{details_heading}":e?i.package_details_heading:i.details_heading,"{trip_label}":i.trip_label,"{package_label}":i.package_label,"{date_label}":i.date_label,"{nation_label}":i.nation_label,"{guests_label}":i.guests_label,"{extras_label}":i.extras_label,"{total_label}":i.total_label,"{waiting_confirmation}":i.waiting_confirmation};let p=e?i.package_message:i.full_message;for(const[u,h]of Object.entries(c))p=p.split(u).join(h);return p},sendToWhatsApp:()=>{const t=a.state,n=document.getElementById("live-total-price");n&&n.textContent;const e=a.utils.getLang(),i=a.utils.getI18n(e),l=t.apiData&&(String(t.apiData.type||"").toLowerCase()==="package"||String(t.apiData.category||"").toLowerCase()==="bundles"),r=a._generateWhatsAppMessage(t,i,l);window.open(`https://wa.me/201063239261?text=${encodeURIComponent(r)}`,"_blank");const d=document.getElementById("invoice-modal");d&&d.classList.add("hidden")}};window.BookingManager=a;const w=`<section id="faq-accordion" class="max-w-5xl mx-auto px-4 mt-16 lg:mt-24 mb-16">
...
</section>`;console.log("🚀 FAQ Module Loaded");console.log("📄 HTML Content Length:",w.length);function $(t="faq-section-container"){console.log("🏁 initFaqSection called for:",t);const n=document.getElementById(t);if(!n){console.error("❌ Error: Container element not found:",t);return}if(n.dataset.loaded==="true")return;n.innerHTML=w,n.dataset.loaded="true";const e=()=>localStorage.getItem("fabio_lang")||document.documentElement.lang||"it",i=d=>d==="en"?window.i18nEn||{}:window.i18nIt||{},l=(d,s)=>{if(!d||!s)return null;const o=s.split(".");let c=d;for(let p=0;p<o.length;p+=1){const u=o[p];if(c&&Object.prototype.hasOwnProperty.call(c,u))c=c[u];else return null}return c},r=()=>{const d=e(),s=i(d);n.querySelectorAll("[data-i18n]").forEach(o=>{const c=o.getAttribute("data-i18n"),p=l(s,c);typeof p=="string"&&(o.textContent=p)})},f=()=>{if(window.DetailsRenderer&&typeof window.DetailsRenderer.setupFaqAccordion=="function"){window.DetailsRenderer.setupFaqAccordion();return}const d=n.querySelectorAll(".faq-item");d.length&&d.forEach(s=>{const o=s.querySelector(".faq-toggle"),c=s.querySelector(".faq-answer"),p=s.querySelector(".faq-icon");!o||!c||o.dataset.bound!=="true"&&(o.dataset.bound="true",o.setAttribute("aria-expanded","false"),c.style.maxHeight="0px",s.classList.remove("is-open"),o.addEventListener("click",()=>{if(o.getAttribute("aria-expanded")==="true"){o.setAttribute("aria-expanded","false"),c.style.maxHeight="0px",s.classList.remove("is-open"),p&&p.classList.remove("rotate-45");return}o.setAttribute("aria-expanded","true"),s.classList.add("is-open"),c.style.maxHeight=`${c.scrollHeight}px`,p&&p.classList.add("rotate-45")}))})};r(),f(),window.addEventListener("langChanged",r)}export{$ as i};
