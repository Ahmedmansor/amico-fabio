const r={state:{hasShownSkeleton:!1},_utils:{getLang:()=>localStorage.getItem("fabio_lang")||document.documentElement.lang||"it",getI18n:e=>e==="en"?window.i18nEn||{}:window.i18nIt||{},resolveCategory:e=>window.ExploreRenderer&&typeof window.ExploreRenderer.resolveCategory=="function"?window.ExploreRenderer.resolveCategory(e):String(e.category||"").toLowerCase().trim(),isPackage:e=>{const t=r._utils.resolveCategory(e);return String(e.type||"").toLowerCase()==="package"||t==="bundles"}},render:e=>{const t=document.getElementById("trips-grid");if(t){if(t.className="trips-grid-container grid grid-cols-1 md:grid-cols-2 gap-4 pb-16 px-3 md:px-4 max-w-full mx-auto",!r.state.hasShownSkeleton&&!window.hasShownSkeleton){r.state.hasShownSkeleton=!0,window.hasShownSkeleton=!0,r._renderSkeleton(t),setTimeout(()=>{r.renderRealCards(e,t)},1500);return}r.renderRealCards(e,t)}},_renderSkeleton:e=>{const t=Array(4).fill(0).map(()=>`
        ...
        `).join("");e.innerHTML=t},renderRealCards:(e,t)=>{t.innerHTML="";const s=e.filter(o=>String(o.featured||"").toUpperCase()==="TRUE");if(!s||s.length===0){r._renderEmptyState(t);return}const a=document.createDocumentFragment(),i=r._utils.getLang();s.forEach((o,l)=>{const n=r.createTripCard(o,i,l);n&&a.appendChild(n)}),t.appendChild(a)},_renderEmptyState:e=>{const t=r._utils.getLang(),s=r._utils.getI18n(t),a=s&&s.global&&typeof s.global.no_exclusive_experiences=="string"?s.global.no_exclusive_experiences:"";e.innerHTML=`<div class="col-span-full text-center py-20">
            <p class="text-gray-400 text-xl font-playfair italic" data-i18n="global.no_exclusive_experiences">${a}</p>
        </div>`},createTripCard:(e,t,s)=>{const a=r._utils.getI18n(t),i=r._utils.isPackage(e),{title:o,description:l}=r._resolveTextData(e,a),{priceRowHTML:n,dealBannerHTML:d}=r._resolvePriceData(e,a,i),b=r._resolveImage(e,i),g=r._resolveBadge(e,t,a),p=(typeof window<"u"&&window.location&&window.location.pathname?window.location.pathname.split("/"):[]).filter(Boolean)[0]||"",f=p?`/${p}/`:"/",m=typeof window<"u"&&window.FABIO_BASE_URL||f,h=window.ImagePaths?window.ImagePaths.ui.placeholder:`${m}assets/images/ui/placeholder.webp`,_=window.ImagePaths?window.ImagePaths.ui.fallbackLogo:`${m}assets/images/logo/logo-fabio-square.webp`,x=a.global&&a.global.price_from?a.global.price_from:"",v=a.global&&a.global.discover?a.global.discover:"",$=a.global&&a.global.discover_package?a.global.discover_package:v,k=i?$:v,y=i?"global.discover_package":"global.discover",L=a.global&&a.global.premium_package?a.global.premium_package:"Premium Package",S=e.package_id||e.trip_id||"",w=encodeURIComponent(S),I=i?`package-details.html?id=${w}`:`details.html?id=${w}`,u=["catalog-card","trip-card"];i&&u.push("premium-package-card");const C=i?" standard-badge-premium":"",T=`
            <article class="${u.join(" ")}" data-trip-id="${e.trip_id}">
                <div class="catalog-card-image">
                    <img src="${b}" alt="${o}" class="catalog-card-img"
                         loading="lazy" onerror="this.onerror=function(){this.onerror=null; this.src='${_}';}; this.src='${h}';">
                ...
                <div class="card-content">
                    ...
                    <div class="card-footer">
                        <div class="price-block">
                            <span class="label-start" data-i18n="global.price_from">${x}</span>
                            ${n}
                        </div>
                        <button class="card-btn" onclick="sessionStorage.setItem('fabio_nav_source','details'); window.location.href = '${I}'" data-i18n="${y}">${k}</button>
                    </div>
                </div>
            </article>
        `,c=document.createElement("div");return c.className="h-full w-full",c.innerHTML=T.trim(),c.firstElementChild},_resolveTextData:(e,t)=>{const s=e.trip_id||e.package_id||"",a=t.trips&&t.trips[s]?t.trips[s]:null;return{title:a?a.title:(e.trip_id||"").replace(/_/g," "),description:a?a.short_desc:""}},_resolvePriceData:(e,t,s)=>{const a=parseFloat(e.p_adult)||0,i=parseFloat(e.d_adult)||0;let o="",l="";if(i>0){const n=a>0?Math.round((a-i)/a*100):0,d=t.global&&t.global.limited_time_deal?t.global.limited_time_deal:"";s?l=`
            ...
            `:o='<div class="price-skeleton"></div>';return{priceRowHTML:o,dealBannerHTML:l}},_resolveImage:(e,t)=>{const s=window.ImagePaths?window.ImagePaths.resolveTripContext(e):{location:"",category:"",tripId:e.trip_id||""};let a=window.ImagePaths?window.ImagePaths.getPoster(s.location,s.category,s.tripId):`/amico-fabio/assets/images/trips/${e.trip_id}/poster.webp`;if(t){const i=e.location||e.Location||e.loc||e.Loc||"sharm",o=String(i).toLowerCase().trim(),l=e.package_id||e.trip_id;a=`/amico-fabio/assets/images/packages/${o}/${l}/poster.webp`}return a},_resolveBadge:(e,t,s)=>{const i=(s.global||{}).badges||{};if(e.badge_key){const o=String(e.badge_key).trim();return i[o]||i[o.toLowerCase()]||""}return t==="it"?e.badge_it||"":e.badge_en||""}};window.TripsRenderer=r;const P={render:()=>{const e=document.getElementById("trips-grid");if(!e)return;const t=[{id:"sharm",title:"Sharm El Sheikh",img:"/amico-fabio/assets/images/locations/sharm.jpg"},{id:"cairo",title:"Cairo",img:"/amico-fabio/assets/images/locations/cairo.jpg"},{id:"luxor_and_aswan",title:"Luxor & Aswan",img:"/amico-fabio/assets/images/locations/luxor_aswan.jpg"},{id:"desert",title:"Sinai Desert",img:"/amico-fabio/assets/images/locations/desert.jpg"}];e.className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-5xl px-4 mx-auto";const s=document.createDocumentFragment();t.forEach((a,i)=>{const o=document.createElement("article");o.className="catalog-card trip-card",o.innerHTML=`
                <div class="catalog-card-image">
                    <img src="${a.img}" alt="${a.title}" class="catalog-card-img" loading="lazy"
                         onerror="this.onerror=null; this.src='${window.ImagePaths?window.ImagePaths.ui.fallbackLogo:"/amico-fabio/assets/images/logo/logo-fabio-square.webp"}';">
            ...
            `,s.appendChild(o)}),e.innerHTML="",e.appendChild(s)}};window.LocationRenderer=P;
